    ...
    def jieba_tuples_generator(self):
        ...
        if self.is_jieba:
            ...
            if isinstance(self.content, str):
                ...
                self.jieba_content = list(
                    map(lambda s: ''.join(filter(lambda x: x not in self.punct, s)), self.jieba_content))
                self.jieba_content = list(
                    pd.Series(self.jieba_content)[[i != '' for i in self.jieba_content]].values)  # 清除''
            else:
                self.jieba_content = [list(jieba.cut(i)) for i in self.content]
                self.diction = self.get_idf(self.jieba_content)  # 只有多行的才能计算IDF
                self.jieba_content = list(
                    map(lambda s: ''.join(filter(lambda x: x not in self.punct, s)), sum(self.jieba_content, [])))
                self.jieba_content = list(
                    pd.Series(self.jieba_content)[[i != '' for i in self.jieba_content]].values)  # 清除''
    ...
    def wordsGenerator(self, base_text, mode=1):
        ...
        if mode == 1:
            for i in tqdm(range(1, len(base_text) - 3)):
                ...
                self.idf_diction[base_text[i] + ' ' + base_text[i + 1]] = np.hstack(
                    (self.diction[(self.diction['words'] == base_text[i])]['idf'].values, \
        ...
        if mode == 2:
            for i in tqdm(range(1, len(base_text) - 4)):
                ...
                self.idf_diction[base_text[i] + ' ' + base_text[i + 1] + ' ' + base_text[i + 2]] = np.hstack(
                    (self.diction[(self.diction['words'] == base_text[i])]['idf'].values, \
        ...
        if mode == 3:
            for i in tqdm(range(1, len(base_text) - 3)):
                ...
                self.idf_diction[base_text[i - 1][-1] + ' ' + base_text[i]] = np.hstack(
                    (self.diction[(self.diction['words'] == base_text[i])]['idf'].values, \
                     self.diction[(self.diction['words'] == base_text[i - 1])]['idf'].values)).mean()
                self.idf_diction[base_text[i] + ' ' + base_text[i + 1][0]] = np.hstack(
                    (self.diction[(self.diction['words'] == base_text[i])]['idf'].values, \
                     self.diction[(self.diction['words'] == base_text[i + 1])]['idf'].values)).mean()

        if mode == 4:
            for i in tqdm(range(1, len(base_text) - 5)):
                base_word.append((base_text[i - 1],
                                  base_text[i] + ' ' + base_text[i + 1] + ' ' + base_text[i + 2] + ' ' + base_text[
                                      i + 3], base_text[i + 4]))
                # 计算idf
                # print('word generator mode 4 ,Round at %s ...' %i)
                self.idf_diction[
                    base_text[i] + ' ' + base_text[i + 1] + ' ' + base_text[i + 2] + ' ' + base_text[i + 3]] = \
...
if __name__ == '__main__':
    ...
    # content = data['new_title'][:10000]
    filepath = '2018gov.txt'
    # data = open('2017gov.txt',mode='r+')     word>= '\u4e00' and word<= '\u9fff'
    data = [line.strip() for line in open(filepath, 'r', encoding='utf-8').readlines() if len(line) >1]

    # ----------- 未分词 -----------
    # generator = termsRecognition(content=data['new_title'][:10000], is_jieba=False, topK=20, mode=[1])  # 文字版
    generator = termsRecognition(content=data, is_jieba=False, topK=30, mode=[1])  # 文字版
    # 全部发现
    result_dict = generator.generate_word()
    result_dataframe = generator.get_result()
    result_dataframe.to_csv('term_result_all_2018.csv',index=False,encoding='gb18030')
    # 部分发现
    result_dataframe = generator.part_found()
    result_dataframe.to_csv('term_result_part_2018.csv',index=False,encoding='gb18030')
